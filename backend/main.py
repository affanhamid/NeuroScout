...
def test_player_observations():
    try:
        ...
        if not player_df.empty:
            ...
            print(f"Organization ID: {player['organizationId']}")
            
            game_obs = player['gameObservations']
            ...
            if game_obs:
                for game_id, game_data in game_obs.items():
                    ...
                    if game_id == ARROW_GAME_ID: #checks if its an arrow game to test with the analyze_arrow_game function
